...
$mod=@return_edefualt(str_callback_w($_GET['mod']),'products');
$file=@return_edefualt(str_callback_w($_GET['file']),'products');
...
switch($action)
{
	...
	case 'config':
		if(isset($dosubmit))
		{
			...
			operation_tips('操作成功！','?mod=products&file=products&action=config');
		}
	...
	//case 'manage':
		...
		//在此写 phpwechat.php?mod=products&file=products&action=manage 时的逻辑
			
		//break;

	//case 'add':
			
		//在此写 phpwechat.php?mod=products&file=products&action=add 时的逻辑
	...
	//case '...':
		...
		//在此写 phpwechat.php?mod=products&file=products&action=... 时的逻辑
