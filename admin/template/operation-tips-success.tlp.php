...
<script src="{$PW['d_path']}statics/jquery/jquery-1.12.2.min.js" language="javascript"></script>
<script src="{$PW['d_path']}statics/core.js" language="javascript"></script>
...
<body>
	<div class="operation_tips">
    	<div class="tips">{php echo $result=='success'?'':'<font class="error">';}{$msg}{php echo $result=='success'?'':'</font>';}</div>
        {if !$flag}<div class="url"><a href="{$redirecturl}">如果您的浏览器没有跳转，请点击这里</a></div>{else}<div class="url"><a href="https://www.so.com/s?ie=utf-8&shb=1&src=home_so.com&q={$msg}" target="_blank">您可以通过该链接，查看相关信息</a></div>{/if}
    </div>
</body>
</html>
