...
<body>
    <div class="ifame-main-wrap">
        <div class="crumb-wrap">
            <div class="crumb-list"><i class="icon-font"></i><a href="{__PW_PATH__}{__ADMIN_FILE__}?file=index&action=main">管理首页</a><span class="crumb-step">&gt;</span><span class="crumb-name">编辑{if !$parentid}区域{else}商圈{/if}</span></div>
        </div>
        <div class="result-wrap">
            <form action="" method="post" name="mysubmitform" id="mysubmitform" enctype="multipart/form-data">
            	<input type="hidden" value="1" name="dosubmit">
                <input type="hidden" value="{intval($id)}" name="id">
                <div class="config-items">
                    <div class="config-title">
                        <h1><i class="icon-font">&#xe026;</i>编辑{if !$parentid}区域{else}商圈{/if}</h1>
                    </div>
                        ...
                        <table width="100%" class="insert-tab">
                            <tbody>
                            	<tr class="formtr">
                                    ...
                                    <td class="formtd">
                                    {php echo Area::getCity($_data['cityid'],'name')}                             
                                    </td>
                                </tr>
                            	<tr class="formtr">
                                    <th class="formth" width="20%"><i class="require-red">*</i>{if !$parentid}区域{else}商圈{/if}名称：</th>
                                    <td class="formtd">
                                 	<input type="text" class="common-text" name="info[name]" value="{$_data['name']}" size="24" />
                                    </td>
                                </tr>
                                {if $parentid}
                                <tr class="formtr">
                                    ...
                                    <select name="info[parentid]">
                                        ...
                                        <option value="{$r['id']}" {php echo $_data['parentid']==$r['id']?'selected':'';} >{$r['name']}</option>
                                        {/loop}
                                    </select>                                    
                                    </td>
                                </tr>
