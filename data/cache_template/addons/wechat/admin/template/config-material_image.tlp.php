...
<body>
    <div class="ifame-main-wrap">
        ...
        <div class="material-list">
			<h2>素材管理</h2>
			<div class="nav">
				<a href="<?php echo defined('PW_PATH')?PW_PATH:'{__PW_PATH__}';?><?php echo defined('ADMIN_FILE')?ADMIN_FILE:'{__ADMIN_FILE__}';?>?mod=wechat&file=config&action=material">图文素材</a>
				<a href="<?php echo defined('PW_PATH')?PW_PATH:'{__PW_PATH__}';?><?php echo defined('ADMIN_FILE')?ADMIN_FILE:'{__ADMIN_FILE__}';?>?mod=wechat&file=config&action=material_image" class="hover">图片</a>
			...
			<div class="btn-post">
				...
				<a href="<?php echo defined('PW_PATH')?PW_PATH:'{__PW_PATH__}';?><?php echo defined('ADMIN_FILE')?ADMIN_FILE:'{__ADMIN_FILE__}';?>?mod=wechat&file=config&action=images_material_synchronism" class="a-btn-post" style="width:165px"><i class="icon-font">&#xe025;</i> 一键同步图片素材库</a>
			</div>
			<?php if($data) { ?>
			<ul id="container">
			<?php $no=1;if(is_array($data))foreach($data as $r){?>
				<li>
					...
					<div class="appmsg_thumb_wrp" style="background-image:url('<?php echo format_url($r['local_url']);?>')"></div>
					<div class="modify"><a href="<?php echo isset($r['local_url'])?$r['local_url']:'';?>" target="_blank" title="查看原图" class="view"><i class="icon-font">&#xe033;</i></a><a onClick="if(!confirm('确定删除该图片素材吗？')){return false;}" href="<?php echo defined('PW_PATH')?PW_PATH:'{__PW_PATH__}';?><?php echo defined('ADMIN_FILE')?ADMIN_FILE:'{__ADMIN_FILE__}';?>?mod=wechat&file=config&action=material_delete&type=Image&id=<?php echo isset($r['id'])?$r['id']:'';?>" class="delete"><i class="icon-font">&#xe061;</i></a></div>
				</li>
			<?php $no++;}?>
			</ul>
